...
<body>
	...
	<br><br><br><br><br>
		...
		<table id = "table" class = "table table-bordered"  style="background-color: white; float:left">
			...
			<tbody>
				<?php
				$act_query = $con->query("SELECT * FROM `archive_student`");
				...
				<tr>
                    ...
                    <td><center><a href = "#" data-toggle = "modal" data-target = "#delete_activity" name = "<?php echo $act_fetch['ID']?>" class = "btn btn-danger ID" style='background-color:gray; border:none'>Delete</a></center></td>
	...
	$(document).ready(function(){
		$('.ID').on('click', function(){
			...
			$('.delete_activity').on('click', function(){
				window.location = 'delete_archive_student.php?ID=' + $ID;
			});
		})
	});
</script>
